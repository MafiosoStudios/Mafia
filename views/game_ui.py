        ...
        if target_state and target_state.role_key == "eren_jaeger" and session.metadata.get("rumbling_active"):
            eren_immune_view = build_v2_layout(
                ...
                description=(
                    ...
                    f"**Eren Jaeger** (<@{target_id}>) has unleashed **The Rumbling** and is "
                    f"**permanently immune to all votes, trials, and executions**!\n\n"
                    f"Deadly Sentencing has failed — the world is about to be flattened."
                ),
                color=discord.Color.dark_red(),
            )
            await self.engine.bot.message_queue.send(mafia_channel, view=eren_immune_view)
            async with self.engine._lock:
                session.metadata["deadly_sentencing_active"] = False
